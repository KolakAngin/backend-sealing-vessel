...
test("Transaksi sealing end-to-end dan audit log", async () => {
  ...
  let vesselId = ""; let terminalId = ""; let categoryId = ""; let templateId = ""; let compartmentId = ""; let pointId = "";
  ...
  try {
    ...
    const created = await call<{ id: string; status: string }>("/reports", json("POST", { reportNo: `RPT-${suffix}`, vesselId, terminalId, cargo: "Crude Oil", operationType: "LOADING", reportDateTime: new Date().toISOString(), portName: "Balikpapan" }));
    ...
    const submitted = await call<{ status: string }>(`/reports/${reportId}/submit`, json("POST", {})); assert.equal(submitted.response.status, 200); assert.equal(submitted.body.data.status, "SUBMITTED");
    ...
    const verifiedSeal = await call<unknown>(`/seals/${replacement.body.data.id}/verify`, json("POST", { condition: "GOOD", remarks: "Seal intact" }), admin.accessToken); assert.equal(verifiedSeal.response.status, 201);
    const verified = await call<{ status: string }>(`/reports/${reportId}/verify`, json("POST", {}), admin.accessToken); assert.equal(verified.response.status, 200); assert.equal(verified.body.data.status, "VERIFIED");
    const approved = await call<{ status: string }>(`/reports/${reportId}/approve`, json("POST", { remarks: "Approved" }), admin.accessToken); assert.equal(approved.response.status, 200); assert.equal(approved.body.data.status, "APPROVED");

    const rejectDraft = await call<{ id: string }>("/reports", json("POST", { reportNo: `REJ-${suffix}`, vesselId, terminalId, operationType: "DISCHARGING", reportDateTime: new Date().toISOString() })); assert.equal(rejectDraft.response.status, 201); reportIds.push(rejectDraft.body.data.id);
    const rejectRecord = await call<unknown>(`/reports/${rejectDraft.body.data.id}/records`, json("POST", { vesselSealingPointId: pointId, status: "NOT_APPLICABLE" })); assert.equal(rejectRecord.response.status, 201);
    assert.equal((await call<unknown>(`/reports/${rejectDraft.body.data.id}/submit`, json("POST", {}))).response.status, 200);
    const rejected = await call<{ status: string }>(`/reports/${rejectDraft.body.data.id}/reject`, json("POST", { remarks: "Rejected in test" }), admin.accessToken); assert.equal(rejected.response.status, 200); assert.equal(rejected.body.data.status, "REJECTED");
